...
public interface ContextualCommandInvoker {

  ContextualCommandInvoker LOCAL_CONTEXTUAL_COMMAND_INVOKER = new Builder.ForLocal().build();
  ...
  static ContextualCommandInvoker local() {
    return LOCAL_CONTEXTUAL_COMMAND_INVOKER;
  }

  static ContextualCommandInvoker empty() {
    return new PrefixStyle(null, Collections.emptyList());
  }

  static ContextualCommandInvoker prefixStyle(String command, String... options) {
    return new PrefixStyle(command, asList(options));
  }

  static ContextualCommandInvoker ssh(String user, String host) {
    return ssh(user, host, null);
  }

  static ContextualCommandInvoker ssh(String user, String host, String identity) {
    return new Builder.ForSsh(host).userName(user).identity(identity).build();
  }


  class ExecutionContextStyle implements ContextualCommandInvoker {
  ...
  record PrefixStyle(String program, List<String> options) implements ContextualCommandInvoker {
  ...
  abstract class Builder<B extends Builder<B>> {
    ...
    public ContextualCommandInvoker build() {
